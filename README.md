...
![Accuracy](https://img.shields.io/badge/Internal%20hold--out%20accuracy-0.7500-brightgreen)
...
| Model | Feature Dim | Internal Hold-out Accuracy | Internal Hold-out F1 | Internal Hold-out AUROC |
|---|---:|---:|---:|---:|
...
improved research coverage but did not improve internal hold-out accuracy
reliably.
...
This repository includes `results.json`, generated by the official runner for
the final fixed-seed run.
...
The final `predictions.csv` is generated by `python solution.py` and should be
uploaded separately as a public file for the application form.
...
- A reproducibility check with fixed random seeds showed that the compact MLP
  representation was the most reliable final choice.
